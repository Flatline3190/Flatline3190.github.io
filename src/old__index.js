...
import './obj_detect/object_detection.js'
import { BrowserRouter as Router, Route } from 'react-router-dom'
import { Element, create3dPage } from './test'
...
//Article()
getnames.load('/in_spot.json', function (all) {
  ...
  for (let i = 0; i < 3; i++) {
    InfoModelize(scene, resourceData[i], gltfs, spots, labs)
...
//vid2(camera, scene, renderer, controls, { x: 111.343, y: 0, z: 113.824 })
//vid(camera, scene, renderer, controls, { x: 111.343, y: 0, z: 113.824 })
nowscene = scene
...
function clickspot(event) {
  ...
  if (intersect_spot.length > 0) {
    console.log(intersect_spot[0].object.name)
    new TWEEN.Tween(camera.position).to(
      {
        x: intersect_spot[0].object.position.x - 3,
        y: intersect_spot[0].object.position.y + 5,
...
function clickmodel(event) {
  ...
  if (intersects.length > 0) {
    ...
    if (concerntrate === 0) {
      ...
      newscene = Amodel(renderer, a)
      const spoting = scene.getObjectByName(a.name + '_spotinfo').clone()
      const labing = scene.getObjectByName(a.name + '_labinfo').clone()
      ...
      //vid2(camera, newscene, renderer, controls, { x: 111.343, y: 0, z: 113.824 })
...
function render() {
  ...
  mvRenderer.render(nowscene, camera)
