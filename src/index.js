...
const renderer = new THREE.WebGLRenderer({ frame, antialias: true, alpha: true })
...
//labelRenderer.domElement.style.bottom = '461px'
//labelRenderer.domElement.style.left = '557px'
...
mvRenderer.domElement.style.position = 'absolute'
mvRenderer.domElement.style.top = '0px'
//mvRenderer.domElement.style.bottom = '461px'
//mvRenderer.domElement.style.left = '557px'

mvRenderer.domElement.style.pointerEvents = 'none'

//document.body.appendChild(mvRenderer.domElement)
container.appendChild(mvRenderer.domElement)
//const webbing = document.createElement('div')
//webbing.id = 'web'
//const roots = document.createElement('div')
//roots.id = 'root'
//webbing.appendChild(roots)
//container.appendChild(webbing)
...
getnames.load('/-surround.json', function (all) {
  ...
  //surroundData.length;
  for (let i = 0; i < 1; i++) {
    console.log(surroundData[i].path)
    gltfloader.load(
      surroundData[i].path,
      function (gltf) {
    ...
    if (build !== undefined) {
      ...
      if (a !== undefined) {
        ...
        if (ok === 0) {
          ...
          //renderer.setViewport(window.innerWidth * 0.25, window.innerHeight * 0.125, window.innerWidth * 0.75, window.innerHeight * 0.75)
          ...
          labelRenderer.setSize(ofst.z, ofst.w)
          mvRenderer.setSize(ofst.z, ofst.w)
          labelRenderer.domElement.style.bottom = ofst.y + 'px'
          mvRenderer.domElement.style.bottom = ofst.y + 'px'

          //labelRenderer.domElement.style.left = ofst.x + 'px'
          //mvRenderer.domElement.style.left = ofst.x + 'px'
          labelRenderer.domElement.style.right = '0px'
          mvRenderer.domElement.style.right = '0px'
...
function onFacilitymove(event) {
  ...
  raycaster.setFromCamera(
    {
      x: ((event.clientX - a.x) / a.z) * 2 - 1,
      y: (-(event.clientY - (renderer.domElement.clientHeight - a.w - a.y)) / a.w) * 2 + 1,
    },
    camera,
  )
...
function clickelse(event) {
  ...
  if (intersect_facility.length === 0) {
    ...
    if (clicked === 1) {
      ...
      for (let i = 0; i < fPanel.children.length; i++) {
        fPanel.children[i].style.display = 'none'
      }
...
function clickfacility(event) {
  ...
  if (intersect_facility.length > 0) {
    ...
    if (document.getElementById(intersect_facility[0].object.name) !== null) {
      document.getElementById(intersect_facility[0].object.name).style.display = 'block'
      console.log(document.getElementById(intersect_facility[0].object.name).style.display)
    }
...
function Backsize() {
  renderer.setViewport(0, 0, window.innerWidth, window.innerHeight)
  labelRenderer.setSize(window.innerWidth, window.innerHeight)
  labelRenderer.domElement.style.right = ''
  labelRenderer.domElement.style.bottom = ''
  labelRenderer.domElement.style.position = 'absolute'
  labelRenderer.domElement.style.top = '0px'

  mvRenderer.setSize(window.innerWidth, window.innerHeight)
  mvRenderer.domElement.style.right = ''
  mvRenderer.domElement.style.bottom = ''
  mvRenderer.domElement.style.position = 'absolute'
  mvRenderer.domElement.style.top = '0px'
}
function Showall() {
  Backsize()
  if (mvscene.getObjectByName('mv_lilac') !== undefined) {
    mvscene.getObjectByName('mv_lilac').visible = false
  }
...
const baseElement = document.getElementById('baseinfo')
rootElement.width = window.innerWidth
rootElement.hieght = window.innerHeight
...
console.log(rootElement.style)
console.log(rootElement.style.zIndex)
console.log(container.style.zIndex)

console.log(baseElement.style)
console.log(baseElement.style.zIndex)
